...
**Rings.** Every completion of a lesson adds one whole ring (×1, ×2, … each its own colour); rings are only ever lost whole. Each missed day every lesson rolls a die: 100% minus 10% per completion is the chance of losing one ring (immune after ten) — so a batch of lessons fades gradually, not as a wall. A unit whose rings all fade stops counting toward your rank and reappears on the Path until you redo it. HSK core units unlock in order — finish one to open the next. Repeating a lesson drills every word, not just the new ones.
