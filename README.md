...
**Lesson strength.** The first completion of a lesson fills its ring to 100% (×1); the next tier takes two completions, and every tier after that takes four — so ×3 → ×4 is four passes whether you're at 300% or 1000%. The ring shows the previous tier's colour underneath and the current tier's colour filling over it. Themes: every ring loses a flat 50% per missed day while each completion adds less as the ring grows (100% at ×0, 50% at ×1, 25% from ×2 up) — so you can stack completions in one sitting to bank days off. HSK: only the lesson you completed most recently fades, one completion's worth per missed day; earlier HSK lessons freeze at the strength they had when you moved on. HSK core units unlock in order — finish one to open the next. Repeating a lesson drills every word, not just the new ones.
