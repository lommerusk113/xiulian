...
*Xiūliàn* — to cultivate. Progress is a cultivation ladder of 33 stages: 凡人 Mortal → 聚气 Qi Building 1–10 (HSK 1 words started) → 炼气 Qi Refining 1–10 (HSK 1 words known) → 筑基 Foundation, 金丹 Golden Core, 元婴 Nascent Soul each in 初期 early / 中期 mid / 后期 late / 圆满 peak (10/40/70/90% of HSK 2/3/4 known, in order). Home shows the current stage, one bar to the next, and the whole path. A keyboard-free Mandarin (simplified) learning app for going from zero to reading web novels in [Readibu](https://readibu.com) and following donghua / manhua. Vue 3 · TypeScript · Tailwind v4 · Nuxt UI 4 · ts-fsrs. Works as an installable PWA on phone and desktop. Progress syncs to an account (Micronaut + Postgres, see below) and is cached in the browser for offline use.
