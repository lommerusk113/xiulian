...
**Lesson strength.** The first completion of a lesson fills its ring to 100% (×1); the next tier takes two completions, and every tier after that takes four — so ×3 → ×4 is four passes whether you're at 300% or 1000%. The ring shows the previous tier's colour underneath and the current tier's colour filling over it. Only the lesson you completed most recently in each track fades: at midnight it loses one completion's worth (100% at ×0, 50% at ×1, 25% from ×2 up), so repeating it every day holds the level and each skipped day costs one completion. Earlier lessons freeze at the strength they had when you moved on. HSK core units unlock in order — finish one to open the next. Repeating a lesson drills every word, not just the new ones.
