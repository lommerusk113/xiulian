...
const props = defineProps<{ options: string[]; correct: string; hanzi?: boolean }>()
...
<template>
  <div class="grid gap-3" :class="hanzi ? 'grid-cols-2' : 'grid-cols-1'">
    <button
      v-for="(o, i) in options"
      :key="o"
      class="relative border-2 rounded-xl px-4 py-4 text-left transition-colors min-h-16"
      :class="[color(o), hanzi ? 'hanzi text-3xl text-center py-6' : 'text-base']"
      @click="pick(o)"
    >
