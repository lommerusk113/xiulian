...
const snap = { cards: {} as Record<string, string>, lessons: {} as Record<string, string>, challenges: {} as Record<string, string>, history: 0, settings: '' }
let timer: ReturnType<typeof setTimeout> | undefined
let inflight = false
let dirty = false

function snapshot() {
  for (const s of ['cards', 'lessons', 'challenges'] as const) {
...
export async function startSync() {
  ...
  try {
    ...
    if (empty) {
      ...
      snapshot()
      snap.cards = snap.lessons = snap.challenges = {}
      snap.history = 0
      snap.settings = ''
    } else {
      Object.assign(progress, { cards: remote.cards, lessons: remote.lessons, challenges: remote.challenges, history: remote.history, settings: { ...progress.settings, ...remote.settings } })
      snapshot()
    }
  } catch {
