...
const snap = { cards: {} as Record<string, string>, lessons: {} as Record<string, string>, challenges: {} as Record<string, string>, tribulations: {} as Record<string, string>, marks: {} as Record<string, string>, retention: {} as Record<string, string>, history: 0, settings: '' }
let timer: ReturnType<typeof setTimeout> | undefined
let inflight = false
let dirty = false

function snapshot() {
  for (const s of ['cards', 'lessons', 'challenges', 'tribulations', 'marks', 'retention'] as const) {
...
async function flush() {
  ...
  const sent = { cards: ser(d.cards), lessons: ser(d.lessons), challenges: ser(d.challenges), tribulations: ser(d.tribulations), marks: ser(d.marks), retention: ser(d.retention), history: d.history?.length ?? 0, settings: d.settings ? JSON.stringify(d.settings) : null }
  try {
    ...
    Object.assign(snap.retention, sent.retention)
...
export async function startSync() {
  ...
  try {
    ...
    if (empty) {
      ...
      snapshot()
      snap.cards = snap.lessons = snap.challenges = snap.tribulations = snap.marks = snap.retention = {}
      snap.history = 0
      snap.settings = ''
    } else {
      Object.assign(progress, { cards: remote.cards, lessons: remote.lessons, challenges: remote.challenges, tribulations: remote.tribulations ?? {}, marks: remote.marks ?? {}, retention: remote.retention ?? {}, history: remote.history, settings: { ...progress.settings, ...remote.settings } })
      snapshot()
    }
  } catch {
